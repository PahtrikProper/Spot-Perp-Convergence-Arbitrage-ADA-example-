...
- **Exit compression:** when in a position, exits on the first of the equity guards or when the basis compresses to `max(EXIT_BASIS_PCT, open_basis * EXIT_COMPRESSION_FRACTION)`, with `EXIT_BASIS_PCT` defaulting to **0.60%** and `EXIT_COMPRESSION_FRACTION` defaulting to **30%**.
